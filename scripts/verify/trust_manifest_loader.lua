...
print(
  string.format(
    "OK: signer=%s signed_at=%s resolvers=%d active=%d",
    signed.signer or "unknown",
    signed.signed_at or "?",
    #(signed.manifest.resolvers or {}),
    #active
  )
)
for _, r in ipairs(active) do
  print(
    string.format(
      "- id=%s endpoint=%s pubkey=%s validTo=%s",
      r.id or "?",
      r.endpoint or "n/a",
      (r.pubkey or ""):sub(1, 16) .. "...",
      r.validTo or ""
    )
  )
end
