...
local function list_presets(presets)
  for _, p in ipairs(presets) do
    print(
      string.format(
        "%-20s engine=%-9s tags=%s modules=%d  %s",
        p.id,
        p.engine or "any",
        table.concat(p.tags or {}, ","),
        #(p.modules or {}),
        p.label or ""
      )
    )
...
local function score(preset, prompt_tokens)
  ...
  local function bump(val)
    if val and prompt_tokens[string.lower(val)] then
      s = s + 2
    end
  end
  bump(preset.id)
