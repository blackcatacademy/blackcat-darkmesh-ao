...
function Jwt.verify_hs256(token, secret)
  if not token or token == "" or not secret then return false, "missing_token" end
  if not crypto_ok or not crypto.hmac_sha256_hex then return false, "crypto_missing" end
  ...
  if not (ok_h and ok_p) then return false, "decode_failed" end
  return true, payload
end
